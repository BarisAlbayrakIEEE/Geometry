...
namespace GeometryNamespace {
	IMPLEMENT_STANDARD_RTTIEXT(CoordSystem, GeometryObject)

		/// <summary>
		/// Protected ctor to be used in the project which acts like a setter
		/// </summary>
		CoordSystem::CoordSystem(
		arrayS3 theOriginCoords,
		arrayS3 theAxisComponentsX,
		arrayS3 theAxisComponentsY,
		arrayS3 theAxisComponentsZ)
		:
	...
	void CoordSystem::copyBase(const CoordSystem& rhs)
	{
		...
		c_originCoords = GeometryMath::copyArray(rhs.c_originCoords);
		c_axisComponentsX = GeometryMath::copyArray(rhs.c_axisComponentsX);
		c_axisComponentsY = GeometryMath::copyArray(rhs.c_axisComponentsY);
		c_axisComponentsZ = GeometryMath::copyArray(rhs.c_axisComponentsZ);
	}
	...
	bool CoordSystem::equals(ARGCOPY(CoordSystem) theCoordSystem) const {
		if (this == theCoordSystem.get()) return true;
	...
	/// </summary>
	OUTVAL(Vector3D) CoordSystem::getAxisAsVectorX() const {
		return new Vector3D(c_axisComponentsX);
	}

	/// </summary>
	/// Getter - The axis as a vector - Y
	/// </summary>
	OUTVAL(Vector3D) CoordSystem::getAxisAsVectorY() const {
		return new Vector3D(c_axisComponentsY);
	}

	/// </summary>
	/// Getter - The axis as a vector - Z
	/// </summary>
	OUTVAL(Vector3D) CoordSystem::getAxisAsVectorZ() const {
		return new Vector3D(c_axisComponentsZ);
	}

	/// </summary>
	/// Getter - All axes - As Vector - In an array
	/// </summary>
	std::vector<OUTVAL(Vector3D)> CoordSystem::getAxesAsVector() const {
		std::vector<Handle(Vector3D)> axesAsVector;
			...
			UncaughtException)
	{
		...
		Handle(Vector3D) axisVectorX;
		try {
			axisVectorX = new Vector3D(theOriginPoint, thePointOnAxisX);
		}
		catch (GeometryException) {
			throw;
		}
		Handle(Vector3D) axisVectorY;
		try {
			axisVectorY = new Vector3D(theOriginPoint, thePointOnAxisY);
		}
		catch (GeometryException) {
			throw;
		}
			...
			UncaughtException)
	{
		...
		Handle(Vector3D) axisVectorZ = theAxisVectorX->crossProduct(theAxisVectorY);
		if (axisVectorZ.IsNull()) throw ParallelAxisException();
		Handle(Vector3D) axisVectorY = axisVectorZ->crossProduct(theAxisVectorX);
		if (axisVectorY.IsNull()) throw ParallelAxisException();
	...
	/// </summary>
	void CoordSystem::setOriginCoords(arrayS3 theOriginCoords) {
		c_originCoords = theOriginCoords;
	}
	...
	/// </summary>
	OUTVAL(PointBase) CoordSystem::createPoint(int theDimensionCount, arrayS3 theCoords) {
		if (theDimensionCount == DIMENSIONS::D2)
		{
			return Handle(Point2D)(new Point2D(this, theCoords));
		}
		return Handle(Point3D)(new Point3D(this, theCoords));
	}
	...
	/// </summary>
	OUTVAL(Point3D) CoordSystem::rotatePointAboutAxisX(ARGCOPY(PointBase) thePoint, double theAngle) {
	...
	/// </summary>
	OUTVAL(Point3D) CoordSystem::rotatePointAboutAxisY(ARGCOPY(PointBase) thePoint, double theAngle) {
	...
	/// </summary>
	OUTVAL(Point3D) CoordSystem::rotatePointAboutAxisZ(ARGCOPY(PointBase) thePoint, double theAngle) {
	...
	/// </sumrnary>
	OUTVAL(Point3D) CoordSystem::rotatePointBase(
		ARGCOPY(PointBase) thePoint,
		const double theAngle,
		const int theAxis0,
		const int theAxis1,
		const int theAxis2) throw (NullptrException)
	{
		if (thePoint.IsNull())
		{
			throw NullptrException();
		...
		return new Point3D(this, rotatedCoords);
	}
}
